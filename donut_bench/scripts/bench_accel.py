"""Donut accelerated benchmark — SDPA and Flash Attention 2 backends.

Measures preprocessing, encoder, and decoder latency with an acceleration
backend applied. Produces results in the same schema as bench_dataset.py,
with an added 'backend' field for direct comparison.

Usage:
    uv run scripts/bench_accel.py --backend sdpa [--pool 50] [--runs 50]
    uv run scripts/bench_accel.py --backend fa2 --batch_size 4
    uv run scripts/bench_accel.py --backend eager  # baseline (same as bench_dataset.py)
    uv run scripts/bench_accel.py --backend sdpa --save results/sdpa_bs1.json
"""
...
from inference.model import apply_accel, load_model
...
class Settings(BenchSettings):
    model_config = SettingsConfigDict(
        cli_parse_args=True, env_prefix="BENCH_", cli_prog_name="bench_accel"
    )
...
def main():
    ...
    accel_label = apply_accel(model, dev, cfg.backend, cfg.no_patch)
    print(f"Backend: {accel_label}")
    ...
    print(f"Measuring ({cfg.runs} run(s), batch={cfg.batch_size}, backend={cfg.backend})...")
    pre_list: list[float] = []
    enc_list: list[float] = []
    dec_list: list[float] = []
    e2e_list: list[float] = []
    ...
    print("=" * 60)
    print(
        f"  {'device':<{w}} : {dev}  |  batch={cfg.batch_size}  |  N={cfg.runs}"
    )
    if torch.cuda.is_available():
        print(f"  {'GPU':<{w}} : {torch.cuda.get_device_name(0)}")
    print(f"  {'backend':<{w}} : {cfg.backend}")
    print("=" * 60)
    enc_peak_str = (
