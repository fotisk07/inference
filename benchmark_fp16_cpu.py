...
MASK_SHAPE = (NUM_WINDOWS, WINDOW_SIZE * WINDOW_SIZE, WINDOW_SIZE * WINDOW_SIZE)  # (1200, 64, 64)
ITERS = 10
...
ms_f32 = bench("masked_fill  float32 on CPU", lambda: t_f32.masked_fill(t_f32 != 0, -100.0))
ms_f16 = bench("masked_fill  float16 on CPU", lambda: t_f16.masked_fill(t_f16 != 0, -100.0))
print(f"\n  → float16 is {ms_f16/ms_f32:.0f}x slower than float32 on this CPU")

if torch.cuda.is_available():
    t_f16_gpu = t_f16.cuda()
    ms_gpu = bench("masked_fill  float16 on GPU", lambda: t_f16_gpu.masked_fill(t_f16_gpu != 0, -100.0))
...
ms_mask_f32 = bench("get_attn_mask  float32 (patched)", lambda: make_mask(torch.float32))
ms_mask_f16 = bench("get_attn_mask  float16 (original)", lambda: make_mask(torch.float16))
...
print(f"  Swin stage 2 has 14 blocks, 7 with shifted windows → 7 mask calls per forward pass.")
print(f"  Multiply by batch size and number of images for total cost.\n")

for n in (1, 7, 50, 200):
    t_orig = simulate_original(n)
    t_patch = simulate_patched(n, device="cuda" if torch.cuda.is_available() else "cpu")
    print(f"  {n:>4} forward passes — original: {t_orig:7.1f} ms   patched: {t_patch:6.2f} ms   speedup: {t_orig/max(t_patch,0.001):.0f}x")
...
if torch.cuda.is_available():
    ...
    ms_cpu_cast = bench("float32 → float16  on CPU", lambda: mask_f32_cpu.to(dtype=torch.float16))
    ms_gpu_cast = bench("float32 → float16  on GPU", lambda: mask_f32_gpu.to(dtype=torch.float16))
    torch.cuda.synchronize()
    print(f"\n  → Doing the dtype conversion on GPU is {ms_cpu_cast/ms_gpu_cast:.0f}x faster")
    print( "    The patched code sends float32 to GPU and converts there (.to(device, dtype=...))")

# ── Summary ──────────────────────────────────────────────────────────────────

separator("Summary")
print(f"  get_attn_mask float16 (original) : {ms_mask_f16:7.1f} ms per call")
print(f"  get_attn_mask float32 (patched)  : {ms_mask_f32:7.1f} ms  (first call only, then cached)")
print(f"  Subsequent calls (cached GPU)    :    ~0.00 ms")
print()
